...
class Generater(nn.Module):
    def __init__(self, opt):
        ...
        self.encoder_decoder = nn.Sequential(
            ...
            nn.Conv2d(opt.ngf * 8, opt.ngf * 16, 4, 2, 1, bias=False),
            nn.BatchNorm2d(opt.ngf * 16),
            nn.ReLU(inplace=True),
            # state size: (nef*8) x 4 x 4
            nn.Conv2d(opt.ngf * 16, opt.nBottleneck, 4, bias=False),
            # state size: (nBottleneck) x 1 x 1
            nn.BatchNorm2d(opt.nBottleneck),
            nn.ReLU(inplace=True),

            # input is Bottleneck, going into a convolution
            nn.ConvTranspose2d(opt.nBottleneck, opt.ngf * 16, 4, 1, 0, bias=False), #8  32
            nn.BatchNorm2d(opt.ngf * 16),
            nn.ReLU(inplace=True),
            # state size. (ngf*8) x 4 x 4
            nn.ConvTranspose2d(opt.ngf * 16, opt.ngf * 8, 4, 2, 1, bias=False), #4    16
            nn.BatchNorm2d(opt.ngf * 8),
            nn.ReLU(inplace=True),
            # state size. (ngf*4) x 8 x 8
            nn.ConvTranspose2d(opt.ngf * 8, opt.ngf * 4, 4, 2, 1, bias=False), #2    8
            nn.BatchNorm2d(opt.ngf * 4),
            nn.ReLU(inplace=True),
            # state size. (ngf*2) x 16 x 16
            nn.ConvTranspose2d(opt.ngf * 4, opt.ngf * 2, 4, 2, 1, bias=False), #1        4
            nn.BatchNorm2d(opt.ngf * 2),
            nn.ReLU(inplace=True),
            # state size. (ngf) x 32 x 32
            nn.ConvTranspose2d(opt.ngf * 2, opt.ngf, 4, 2, 1, bias=False), #         2
            nn.BatchNorm2d(opt.ngf),
            nn.ReLU(inplace=True),
            # state size. (ngf) x 64 x 64

            nn.ConvTranspose2d(opt.ngf , opt.ngf, 4, 2, 1, bias=False),  # 2
            nn.BatchNorm2d(opt.ngf),
            nn.ReLU(inplace=True),
            # state size. (ngf) x 64 x 64
            nn.ConvTranspose2d(opt.ngf, opt.nc, 4, 2, 1, bias=False),
            nn.Tanh()
            # state size. (nc) x 128 x 128#256
        )
