        ...
        for epoch in range(self.opt.max_epoch):
            ...
            for ii, (imgs, _) in enumerate(progressbar):
                ...
                if (ii + 1) % self.opt.g_every == 0:
                    ...
                    G_loss.update(error_g + self.opt.contrast_loss_weight * error_c)
            ...
            '''
            #loss图
            D_Loss_list.append(d_loss.getavg())
            G_Loss_list.append(G_loss.getavg())
            x1.append(epoch)
            y1 = G_Loss_list
            y2 = D_Loss_list
            plt.clf()
            lines=plt.plot(x1, y1,'m*-', x1, y2, 'o-')
